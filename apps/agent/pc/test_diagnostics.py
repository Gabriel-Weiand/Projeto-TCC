...
def main():
    ...
    parser.add_argument('--check-fields', action='store_true', help='Verifica completude dos campos de telemetria')
    ...
    if not args.api and not args.live and not args.check_fields:
        print('  Dica: use --api para testar conexão, --live para coleta contínua, --check-fields para validar campos')
        print()


def test_field_completeness():
    """Verifica que collect_telemetry() retorna TODOS os campos esperados pela API."""
    print('=' * 60)
    ...
    # Campos obrigatórios (a API requer todos)
    required_fields = [
        'cpuUsage', 'cpuTemp', 'gpuUsage', 'gpuTemp',
        'ramUsage', 'loggedUserName',
    ]

    # Campos opcionais (a API aceita mas não exige)
    optional_fields = [
        'diskUsage', 'downloadUsage', 'uploadUsage', 'moboTemperature',
    ]
    ...
    for field in optional_fields:
        present = field in data
        value = data.get(field, 'não detectado')
        status = '✓' if present else '–'
        print(f'    {status} {field:20s} = {value}')
    ...
    print('\n  Validação de escalas:')
    scale_checks = [
        ('cpuUsage', 0, 1000), ('gpuUsage', 0, 1000),
        ('ramUsage', 0, 1000), ('diskUsage', 0, 1000),
        ('cpuTemp', 0, 1500), ('gpuTemp', 0, 1500),
    ]
    for field, min_val, max_val in scale_checks:
        if field in data and isinstance(data[field], (int, float)):
            val = data[field]
            in_range = min_val <= val <= max_val
            status = '✓' if in_range else '✗'
            print(f'    {status} {field:20s} = {val} (esperado: {min_val}-{max_val})')
            if not in_range:
                all_ok = False
    ...
    if all_ok:
        ...
        print('\n  ✗ Há campos ausentes ou fora da escala!')
