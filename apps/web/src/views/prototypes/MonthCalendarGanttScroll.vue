...
function rowH(idx: number): number {
  if (maxOffset.value === 0) return ROW_H; // ≤8 machines: no scroll, no extension
...
function barTopOffset(idx: number): number {
  const center = Math.round((ROW_H - BAR_H) / 2); // 14px
...
const CELL_W = 46;         // px per day column
const MONTH_HEADER_H = 30; // px — month label row height
const DAY_HEADER_H = 46;   // px — day numbers row height
const ROW_H = 66;           // px — base machine track row height
const BAR_H = 38;           // px — allocation bar height
const BTN_H = 20;           // px — scroll button strip height
const TOTAL_HEADER_H = MONTH_HEADER_H + DAY_HEADER_H;
const TOTAL_DAYS = PAST_DAYS + 1 + FUTURE_DAYS; // 91 days
...
// Today line x: center of today column
const todayLineX = PAST_DAYS * CELL_W + CELL_W / 2;
// Today column left edge (for highlight bg)
...
// Alternating month bg gradient (hard CSS stops)
...
function barsForMachine(machineId: number) {
  ...
  return allocations.value
    ...
    .map((a) => {
      ...
      const widthDays = Math.max(0.85, (ce.getTime() - cs.getTime()) / MS_PER_DAY);
      ...
      return {
        ...
        isOwn: a.isOwn === true || (currentUserId.value !== null && a.userId === currentUserId.value),
...
// ---- Visible month label (for floating "you are here" indicator) ----
const scrollLeft = ref(0);

const visibleMonthLabel = computed(() => {
  // Which month is at the center of the current viewport?
...
<template>
  <div class="fade-in proto-wrap">
    <div class="proto-banner">
      <span class="proto-tag">PROTÓTIPO D</span>
      Gantt Rolagem Contínua — Timeline sem paginação de meses
    </div>

    <div class="page-header">
      <h1 class="page-title">Calendário de Alocações — Gantt Infinito</h1>
    </div>
    ...
    <template v-else>
      ...
      <div class="gantt-outer">
        ...
        <div class="gantt-left" ref="ganttLeftEl" @wheel="onLeftWheel">
          ...
          <div class="left-header" :style="{ height: TOTAL_HEADER_H + 'px' }">
            <span class="left-header-label">
              Máquina
              <span class="machines-range">{{ machineOffset + 1 }}–{{ Math.min(machineOffset + PAGE_SIZE, machinesStore.machines.length) }} / {{ machinesStore.machines.length }}</span>
            </span>
          </div>
        ...
        <div class="scroll-wrapper">
          ...
          <button class="scroll-arrow left" @click="jumpBy(-CELL_W * 5)">‹</button>
          <button class="scroll-arrow right" @click="jumpBy(CELL_W * 5)">›</button>

          <!-- Scrollable container (no visible scrollbar) -->
          <div class="gantt-scroll" ref="scrollEl" @mousedown.prevent="onMouseDown">
            <div
            ...
            >
              ...
              <div
                ...
                :key="machine.id"
                class="track-row"
                :style="{ height: rowH(idx) + 'px', '--bar-top': barTopOffset(idx) + 'px' }"
              >
                ...
                <!-- Month-start vertical separators (injected via CSS background trick) -->
                ...
                >
                  <span class="bar-label">
                    {{ bar.allocation.user?.fullName ?? "Reserva" }}{{ bar.isOwn ? " ✦" : "" }}
                  </span>
                </div>
              </div>
            </div>
      ...
      <!-- Bottom hint -->
      <p class="hint-text">
        💡 Passe o mouse sobre as barras para detalhes · Arraste ou role com o mouse para navegar · Janela: {{ PAST_DAYS }} dias passados + {{ FUTURE_DAYS }} dias futuros
      </p>
    </template>
  </div>
</template>
...
.leg-bar.approved { background: rgba(102,126,234,0.35); border-color: #667eea; }
.leg-bar.pending  { background: rgba(251,191,36,0.3);   border-color: #fbbf24; border-left-style: dashed; }
.leg-bar.finished { background: rgba(255,255,255,0.07); border-color: rgba(255,255,255,0.22); }
.leg-bar.own-leg  {
...
.drag-hint { color: var(--text-muted); }
...
.machine-scroll-btn--up {
  border-bottom: 1px solid var(--border);
  /* top is set via :style binding = TOTAL_HEADER_H px */
}
...
.machine-label-row:last-child { border-bottom: none; }
...
.scroll-wrapper {
  flex: 1;
  position: relative;
  overflow: hidden; /* clip the scroll area */
  min-width: 0;
}
...
.gantt-scroll::-webkit-scrollbar { display: none; } /* Chrome/Safari */
.gantt-scroll:active { cursor: grabbing; }

/* ---- Timeline inner (full width) ---- */
.timeline-inner {
  position: relative;
  /* month alternating bg applied via :style binding */
}

/* Day grid lines via repeating gradient overlay on timeline-inner */
...
.month-span:last-child { border-right: none; }
...
.day-cell.today   { background: rgba(124,108,240,0.1); }
...
.day-cell.today .d-letter { color: var(--accent); }
...
.gantt-bar.own {
  box-shadow: 0 0 0 1.5px rgba(255,255,255,0.6), 0 0 12px rgba(124,108,240,0.5);
  z-index: 3;
}
